...
class CineCalidad :
    ...
    fun videoListParse(response: Response): List<Video> {
        val document = response.asJsoup()
        return document.select("#playeroptionsul li").catchingFlatMapBlocking {
    ...
    private suspend fun serverVideoResolver(url: String): List<Video> {
        ...
        return when {
            ...
            embedUrl.contains("ahvsh") || embedUrl.contains("streamhide") || embedUrl.contains("guccihide") || embedUrl.contains("streamvid") || embedUrl.contains("vidhide") -> VidHideExtractor(client, headers).videosFromUrl(url)
    ...
    override fun animeDetailsParse(document: Document): SAnime = SAnime.create().apply {
        ...
        document.select(".single_left table p > span").map { it.text() }.forEach { textContent ->
            when {
