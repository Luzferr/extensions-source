...
object HentailaFilters {
    ...
    open class CheckBoxFilter(name: String) : AnimeFilter.CheckBox(name) {
        fun toQueryPart(name: String): String = this.state.takeIf { it }?.let { "&$name=true" } ?: run { "" }
    }
    ...
    private inline fun <reified R> AnimeFilterList.checkAsQueryPart(name: String): String = (this.getFirst<R>() as CheckBoxFilter).toQueryPart(name)
